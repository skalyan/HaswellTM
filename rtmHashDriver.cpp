...
executeThreadLoopSimple(void* data)
{
    ...
    for (int uic = updateIters, ec = wArg->nEntries_; ec > 0; --ec, --uic) {
        int k = rand() % wArg->htPtr_->numBuckets();
        unsigned int nacqs = 0;

        TX_BEGIN(GLock, nacqs, MaxRetries)


        if (uic == 0) {
        	wArg->htPtr_->update(k, v);
			uic = updateIters;
		} else {
			if (wArg->htPtr_->lookup(k, v)) {
			   tlSum += v;
            }
		}
...
processArgs(int argc, char *argv[])
{
    ...
    if (wsplit == 0) {
		...
		UpdateSpacing = (NumEntries / ((wsplit * NumEntries)/100)) - 1;
	}
