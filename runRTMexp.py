...
def assembleCommand(syncName, numThreads, numIters, sharedState, collisionFactor, numRetries):
	...
	exeStr = exeName + threads + iters + sharedState + collisionFactor + retries + outFile
	return exeStr

def runOneExperiment(cmdStr):
	if not cmdStr:
		print "Error: experiment command is empty"

	print "Running ", cmdStr
		
	res = Command(cmdStr).run()
	if res.failed:
		print "Error: ", res.error, "; Unable to run an experiment"
	...
	return {'RunTime' : runTime, 'TxStarts': 0, \
		'TxCommits' : 0, 'TxAborts' : 0}
...
def runOneSyncExperiment(syncName, numRetries):
	...
	while (collisionFactor > 0):
		...
		print "Config-> ", syncName, ", ", collisionFactor , ", ", numRetries, ", ", aRunTime
